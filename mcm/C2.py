...
params = {
    ...
    # 杂草模块
    'W_max': 8000.0,
    'mu_W_min': 0.025,  # 降低最小生长率
    'mu_W_amp': 0.008,
    'gamma_W': 0.00,  # 恢复除草剂效率
    'beta_WC': 0.0005,  # 作物抑制系数
    'k': 0.7,
...
def temperature_factor(day):
    """ 温度响应函数优化（10-30℃适宜） """
    T = 20 + 10*np.sin(2*np.pi*(day-80)/365)
    return np.clip((T - 10)/20, 0.0, 1.0)
...
def agricultural_model(t, y):
    ...
    M = np.clip(M, 0.0, 1.0)
    # 杂草种子动态
    dW_seed = -0.02 * W_seed  # 自然衰减率调整为2%/天
    if 60 <= day < 100:  # 春季萌发
        germination = 0.08 * W_seed
        W += germination
        W_seed -= germination
    if 200 <= day < 270:  # 秋季产种
        seed_production = 0.15 * W
...
# ================== 模拟设置 ==================
y0 = [500.0, 400.0, 10.0, 40.0, 2.2,2000.0]
