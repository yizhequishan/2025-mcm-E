...
class HabitatSystem:
    ...
    def update_pesticide(self, day):
        ...
        self.pesticide_residue += self.pesticide_app(day)

    def pesticide_app(self, day):
...
class DynamicOwlModel:
    ...
    def update_occupancy(self, pest_density):
        for patch in self.habitat_patches:
            ...
            patch['occupancy'] = np.clip(
                (1 - extinction) * patch['occupancy'] + colonization * (1 - patch['occupancy']), 0, 1)


# ================== 可视化与保存 ==================
def plot_and_save(grid, habitat_sys, owl_model, day, output_dir):
    ...
    # 保存图片
    plt.tight_layout()
    save_path = os.path.join(output_dir, f"day_{day:03d}.png")
...
def run_full_simulation():
    # 创建输出目录
    output_dir = f"simulation_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for day in range(params.days):
        ...
        # 物种交互
        ag_grid.grid[:, :, 3] = syrphid_model.diffuse(ag_grid.grid, habitat_sys.get_syrphid_habitat())
        owl_model.detect_habitat(habitat_sys.edge_maturity)
        owl_model.update_occupancy(ag_grid.grid[:, :, 2])
        ...
        # 每30天保存结果
        if day % 30 == 0:
            saved_path = plot_and_save(ag_grid.grid, habitat_sys, owl_model, day, output_dir)
            print(f"Day {day:03d} 结果已保存: {saved_path}")

        # 记录数据
        results.append({
            'day': day,
            'crop': np.mean(ag_grid.grid[:, :, 0]),
            'pest': np.mean(ag_grid.grid[:, :, 2]),
            'syrphid': np.mean(ag_grid.grid[:, :, 3]),
            'owl_occ': np.mean([p['occupancy'] for p in owl_model.habitat_patches]) if owl_model.habitat_patches else 0
        })
    ...
    plt.title('Simulation Summary'), plt.grid(True)
    summary_path = os.path.join(output_dir, "final_summary.png")
    plt.savefig(summary_path, dpi=300)
    plt.close()

    print(f"\n模拟完成！所有结果已保存至: {os.path.abspath(output_dir)}")
    return results
